...
- Approach:
  - 默认层提供稳定地标与决策入口，细节按信息线索展开。
  - 降低记忆、搜索、导航和恢复成本，保留比较、核验与纠错。
  - 用直接操纵、可撤销反馈和对照评测支持自主与胜任感。
- Input:
- Output: 人类舒适协作研究与主流进度管理产品对照；见 docs/agent-context-research/human-capability-comfort.md、docs/agent-context-research/progress-management-products.md。
- Metrics: 研究覆盖外部认知、信息觅食、中断恢复、自主性与人机协作；设计假设可用定向、恢复、纠错和信任校准对照验证，阈值由基线与错误代价确定。
- Notes:
- CodeLoc:
- CurrentResult:
  - 人应借助外部表示形成局面、比较和恢复，而非被动读完整树；舒适是降低搜索与恢复成本并保留控制权。
  - 8 个主流产品均以统一对象配合多视图，分开方向健康、项目计划、日常执行与证据详情。
  - 真正可迁移的是稳定入口、情境视图、概要到证据和直接操纵；具体企业层级与自动完成率不能硬套认知理论。
- RootCauseAnalysis: 此前把“少字、快速读完”当作舒适性的代理，仍在要求人维护模型的内部结构。根因是忽略了外部认知、直接操纵、恢复线索和自主性。
- CaseStudy:
- NextIdea: 将竞品共同模式转成少量可观察的界面对照假设，先评估现状再与用户讨论是否改版。
- SelectedSkills:
