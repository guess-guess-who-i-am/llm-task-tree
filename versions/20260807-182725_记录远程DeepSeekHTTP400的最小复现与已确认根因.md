...
- CurrentResult:
  ...
  - 桌面与 390px 回归通过；本机、共享 kit、插件运行时和 huangyu 的 3 个前端文件哈希一致。
  - 远程 Codex 与 task-tree MCP 可用；共享上游 CPU 过载时完整树回合仍可能超时。
- RootCauseAnalysis: 旧回顾直接映射任务树字段和节点清单，导致风险、旁支与过期 Current 抢占注意力。远端剩余不稳定来自共享上游过载，不是 SSH 或任务图服务。
