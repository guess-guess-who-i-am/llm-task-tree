...
const GRAPH_DETAIL_MAX_SCALE = 1.4;
...
els.graphViewport.addEventListener("wheel", (event) => {
  ...
  if (!focusLensOpen && event.deltaY < 0 && nextScale > GRAPH_DETAIL_MAX_SCALE) {
    openFocusLens(resolveLensFocus(event));
    return;
  }
  graphView.scale = clamp(nextScale, 0.28, GRAPH_DETAIL_MAX_SCALE);
