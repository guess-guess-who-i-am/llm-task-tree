...
- Approach:
  - 摘要和宏观节点按核心文字量自适应宽高；排版按细节层高度预留。
  - 详文保留手动尺寸；连线和缩放读取实际尺寸。
- Input:
- Output:
- Metrics:
- Notes:
- CodeLoc:
- CurrentResult: 摘要节点现按内容自适应：宏观6种、细节10种、手机9种尺寸，标题无裁切、节点无重叠；总览和透镜回归通过。仍待真实使用验证，“一眼看清并直接编辑”目标未达到。
- RootCauseAnalysis: 固定 Size 同时支配摘要、宏观和详文，造成短节点空白、长节点受限；宏观排版过紧还会在放大后重叠。
- CaseStudy:
- NextIdea:
- SelectedSkills:
...
- Input:
- Output: 正式界面、测试与截图：public/，scripts/test-tree-first-workspace.mjs，artifacts/tree-first-workspace-*.png。
- Metrics: 能否快速说出项目目标、当前问题和焦点依据；缩放与切换全貌后是否保持方向感。
- Notes:
- CodeLoc:
- CurrentResult: 默认首屏保留20字辅助状态摘要，完整面板不展开；全页可见字389，知识库、版本和执行链状态均可直接判断，桌面和手机回归通过。仍待真实使用验证打开与浏览意愿，舒适使用目标未达到。
- RootCauseAnalysis: 完整辅助面板会挤压画布，完全隐藏又失去状态线索；默认层应只保留可判断的摘要。
- CaseStudy:
