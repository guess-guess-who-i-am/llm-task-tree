...
## 2026-08-17 精炼状态常驻工作台

- 定量确认宏观任务树只有 212 字，主要噪声来自完整展开的知识库、338 条版本记录和执行链。
- 默认首屏不再完全隐藏辅助信息：常驻 20 字状态摘要，可直接判断知识库、版本和执行链是否可用、规模及异常。
- 完整面板默认不展开，按需打开并记住用户布局；全页可见字为 389，中央画布仍是视觉主体。
- 手机摘要进入图谱标题第二行，桌面和手机均未出现新增重叠或横向溢出。
- `scripts/test-tree-first-workspace.mjs`、总览和焦点透镜回归均通过；正式源、共享 kit、插件运行时和本机模板静态资源一致。

## 2026-08-19 持续 Supervisor 与动态运行树

- 初始并行计划仍由用户审核；批准后由同一 Supervisor 会话持续派发独立 Worker，临时任务只进入 `.task-tree-runs/<runId>/execution-tree.json`，不膨胀方法树。
- Worker 完成后 Supervisor 可继续新增分支、进入集成或请求用户决定；用户消息串行进入同一总控会话，并可暂停/恢复自动调度。
- 17/17 Supervisor、协调器与 Codex 运行测试通过；桌面/移动端点击回归、真实 Git 隔离工作区的接受、拒绝、失败重跑均通过。
- 修复状态轮询竞态：暂停/继续或发送消息期间，旧轮询响应不会覆盖最新用户操作；发送失败会恢复输入内容。
- 构建脚本现在以 Codex manifest 为版本源同步 Cursor、Claude 与 Claude marketplace；源、共享 Kit、插件运行时均通过 45 文件一致性检查。
- Codex 桌面缓存仍是旧安装版本，刷新被 WindowsApps 的 `codex.exe` 权限阻断；未手工覆盖宿主缓存，待桌面端重新安装插件后复核。
