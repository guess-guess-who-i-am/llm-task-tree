...
- CurrentResult:
  - 同一载荷可无修改生成完整翻译，确认先前为 Codex/模型瞬时退出；发布器现会自动尝试三次。
  - 桌面入口已改为 wscript.exe 无窗口启动，源文件与本机安装副本哈希一致，相关回归通过。
  - 当前 40 行、33 条规则已发布；本机与 huangyu 中英文哈希一致，复检为 already-current 且两端 Hook 正常。
- RootCauseAnalysis: 同一载荷可无修改成功，说明本次是 Codex/模型瞬时退出；旧发布器缺少内部重试，桌面快捷方式还直接启动了可见 PowerShell。
- NextIdea: 下次从新桌面快捷方式保存一次小改动，确认只出现编辑器和结果提示，不出现终端窗口。
