...
- Notes:
- CodeLoc:
- CurrentResult: 目标是让人无需展开或追问就能看懂每个节点的核心状态。当前已统一显示问题、思路、结果，并将放大后的操作栏压为两行、每项摘要限两行；14 个节点在 1.8 倍细节缩放下实测无内容溢出，宏观模式仍只显示标题。界面可宣称达到快速判断与稳定阅读，模型长期准确写回仍需真实任务验证。
- RootCauseAnalysis: 旧规则要求“方向性结论”，界面又优先显示 ROOT 摘要；压缩时因此丢失阶段目标、当前能力和达成判断。
- CaseStudy:
- NextIdea: 在下一次真实项目推进中审查 CurrentResult 是否保留用户目标、现状、差距和达成判断；若失败，继续修通用规则而非项目特例。
- SelectedSkills:
- Folded: true
