...
- Notes:
- CodeLoc:
- CurrentResult: 持续 Supervisor 已能复用同一总控会话，按运行树动态新增分支、汇总 Worker 结果，并接受用户消息、暂停或恢复；模拟与真实隔离回归通过。主树写入和长期真实协作仍待验证。
- RootCauseAnalysis: 临时并行任务若直接写方法树会污染长期状态；因此运行树隔离任务、写集和状态，由总控在审核边界汇总。
- CaseStudy:
...
- Approach: 方法树管当前目标与执行；支撑树管背景约束；架构树管稳定契约。切换时画布优先显示，附属面板按需并发加载。
- Input:
- Output: 界面与回归：public/、scripts/。
- Metrics: 能否快速说出项目目标、当前问题和焦点依据；缩放与切换全貌后是否保持方向感。
- Notes:
- CodeLoc:
- CurrentResult: 切树不再等待附属面板；人为延迟1.2秒时首次显示216ms、缓存45ms。界面已解释三类树的职责；仍待真实使用反馈，舒适使用目标未达成。
- RootCauseAnalysis: 旧切换串行等待版本、模型、知识库等无关请求，还会写快照；树的职责只写在注册表，界面没有说明。
- CaseStudy:
- NextIdea: 在真实安装项目间往返切换三类树，记录仍会等待或不知道该看哪棵树的场景。
- SelectedSkills:
