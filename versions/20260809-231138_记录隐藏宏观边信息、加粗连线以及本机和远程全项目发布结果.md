...
- CodeLoc:
- CurrentResult:
  - 宏观任务树已收敛为“标题节点＋结构连线”，减少缩小时需要阅读的信息。
  - 14/14 节点保持约 16px、字重 800；宏观边标签和连接点可见数均为 0，放大后恢复编辑。
  - 普通、Current、Next 边宽分别提高到 3.2、6、9；自动验收无控制台错误，正式源、kit 与插件运行时已同步。
- RootCauseAnalysis: 旧界面把缩放只当几何变换，并让活动边与普通边共享接近的视觉权重，因此整树缩小时文字消失、主干难辨。问题核心是缺少随尺度变化的信息层级，而不是需要整体重做界面。
- CaseStudy:
- NextIdea: 把新版共享 kit 发布到本机全部已注册项目和 huangyu 远程 kit，并核验文件哈希与失败数。
- SelectedSkills:
