...
- CodeLoc:
- CurrentResult:
  - “回顾”已改为决策总览：默认显示根本目标、当前局面、阻塞、推荐下一步、2 个可选方向和 13 行节点状态。
  - ◆ 可直接设置 Next；点击候选或节点行会关闭总览、切回关系图并定位节点，详情字段不进入总览。
  - Playwright 已通过桌面 1600×1000 与移动 390×844 验证；截图见 artifacts/project-overview-desktop.png、artifacts/project-overview-mobile.png。
- RootCauseAnalysis: 阅读阻力来自默认入口按“存储字段”组织，而用户需要按“决策优先级”恢复上下文；因此保留完整关系图作二级编辑工具，以决策总览承担默认入口。
- CaseStudy:
- NextIdea: 让用户实际从“总览”进入一次项目，确认是否能在 30 秒内决定继续 N11、切换 N6 或定位任一节点；仅按实测反馈调整信息层级。
- SelectedSkills:
