...
- CurrentResult:
  - 节点中文化与无代码覆盖规则已进入全局中英文提示、项目钩子、Codex 执行提示和安装模板；冻结原协议逐字保留，短入口为 7,822 bytes。
  - 机械门禁允许 LLM、token、API、必要名称、路径和自然案例标签；会拒绝代码、命令、原始数据及英文专业长段，含误报保护的专项测试通过。
  - 本机 29/29 项目同步成功；huangyu 的全局提示、父工作区与插件 runtime 已更新，远端树写作违规为 0。
- RootCauseAnalysis: 旧协议要求在输入输出字段粘贴原始样例，直接诱导 Agent 写入代码和复杂英文。冻结协议需保留审计，因此用优先级更高的覆盖规则与机械门禁纠正。
- NextIdea: 在真实项目观察新门禁的误报；必要时只收窄检测模式，不放宽中文化和无代码原则。
