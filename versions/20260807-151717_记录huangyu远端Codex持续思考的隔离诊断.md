...
- CurrentResult: 桌面 Prompt 发布器已安装；首次真实发布通过 34 行、27 条规则、2 个标题门禁。本机与 huangyu 的中英文哈希一致、Hook 均为 ok；复跑为 already-current 且 translated=false。
- RootCauseAnalysis: 本机和远端都由用户级 UserPromptSubmit Hook 覆盖该账号的全部项目，因此发布目标是各 Codex Home，不需向每个项目目录复制 Prompt。
- NextIdea: 用桌面“编辑并发布全局 Prompt”快捷方式完成一次真实保存，核验编辑器保存事件的端到端触发。
