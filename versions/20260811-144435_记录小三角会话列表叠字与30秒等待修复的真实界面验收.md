...
- Notes:
- CodeLoc:
- CurrentResult: 目标是让 Codex 主动维护核心状态且用户可低成本调用。已定位小三角每次新启 app-server 并翻页导致会话读取 5.7–8.5 秒，执行前固定等待最多 15 秒；现改为后台预热与 30 秒缓存，冷启动菜单立即可发送，连续接口 4–9 毫秒，turn 接受后立即切换（测试约 12 毫秒）。本机性能目标已达到，跨项目发布尚未验证。
- RootCauseAnalysis: 会话菜单把机器级会话分页扫描放在同步请求中，且每次新建 app-server；执行入口又把 15 秒故障观察窗口放在跳转前。根因是把一致性与故障检测放进用户交互关键路径，缺少后台刷新和缓存。
- CaseStudy:
- NextIdea: 在当前界面实际点击确认交互无误，再把同一优化发布到所有已安装项目并抽测本机与远程 Linux。
- SelectedSkills:
- Folded: true
...
- Approach:
  ...
  - 缩小时节点只显示加粗标题，边只表达结构而不显示关系信息；放大后恢复完整编辑。
...
- CodeLoc:
- CurrentResult:
  - 宏观任务树已收敛为“标题节点＋结构连线”：边信息入口隐藏，普通、Current、Next 边宽为 3.2、6、9。
  - 本机注册表 34 项中，29 个真实任务树项目完成共享 stub、规则和提示词刷新，5 个无任务树路径跳过，失败 0。
  - huangyu 的 3 个任务树项目统一指向 /data/fqd/llm-task-tree-kit；本地与远程 4 个关键 UI 文件哈希一致。
- RootCauseAnalysis: 旧界面把缩放只当几何变换，并让活动边与普通边共享接近的视觉权重，因此整树缩小时文字消失、主干难辨。问题核心是缺少随尺度变化的信息层级，而不是需要整体重做界面。
- CaseStudy:
- NextIdea: 请用户刷新本机和远程任务树，验收宏观连线粗细及放大后边编辑是否正常。
- SelectedSkills:
