...
- CurrentResult: 远端实测：将端口标记置为 1 后，task_tree_server 自动恢复 5633；task_tree_open 返回 ui://task-tree/graph.html；446930 B widget 含 host bridge 与 API shim。本机 15633 转发返回 HTTP 200；MCP、Linux kit、share-install 回归全通过。
