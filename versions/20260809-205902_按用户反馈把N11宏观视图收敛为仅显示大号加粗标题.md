...
- CodeLoc:
- CurrentResult:
  - 正式树已能在宏观缩放下保留项目方向和活动主干，放大后恢复执行细节。
  - 14/14 节点在 0.22 缩放时切换摘要，标题实际约 14px；ROOT→Next 主干连续 4 段、7px 实线。
  - 放大后 14/14 节点恢复细节，自动验收无控制台错误；正式源与 kit 已同步。
- RootCauseAnalysis: 旧界面把缩放只当几何变换，并让活动边与普通边共享接近的视觉权重，因此整树缩小时文字消失、主干难辨。问题核心是缺少随尺度变化的信息层级，而不是需要整体重做界面。
- CaseStudy:
- NextIdea: 请用户在正式任务树验收缩小后的宏观方向、主干位置和放大后的细节恢复，再决定是否继续调整节点层次。
- SelectedSkills:
