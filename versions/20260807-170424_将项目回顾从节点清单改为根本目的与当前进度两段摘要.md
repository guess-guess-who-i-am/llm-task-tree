...
- CurrentResult:
  - 已修复代理槽位泄漏并将排队、首字节超时降为 60 秒；本机 4000 为 3.3 秒、远程 4152 为 1.7 秒返回完成结果。
  - 远程 Codex 已更新到稳定版 0.147.0；普通消息返回 OK 且退出码 0，system skills 权限错误已消失。
  - task-tree MCP 可初始化并列出 17 个工具；模型工具格式已增加 fallback，但共享上游 CPU 过载时完整树回合仍可能超时。
- RootCauseAnalysis: 持续思考的主因是断开请求未移出 semaphore 队列、槽位交接计数膨胀，再叠加 900 秒等待。剩余不稳定来自自定义模型的多种伪工具格式和共享上游 503 过载，不是 SSH 或 task-tree 服务端。
- NextIdea: 在远端新开 Codex 任务发送普通消息；若树工具回合仍超时，先关闭占用同一上游的旧任务，再复测文件读取 fallback。
