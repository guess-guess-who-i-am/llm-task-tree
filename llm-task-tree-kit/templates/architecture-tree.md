...
> 本树保存任务图系统的稳定实现架构；方法变化、实验结果和执行状态仍留在 active method tree 与 scripts。
...
## AE1 - 数据契约支撑交互层
- Endpoints: ARCH, A1
- Label: schema 是全部组件的持久化边界。

## AE2 - 编辑器实现数据契约
- Endpoints: A1, A2
- Label: UI 与服务端共同保证 Markdown 往返一致。

## AE3 - 编辑器封装为可移植 Kit
- Endpoints: A2, A9
- Label: kit 分发同一运行时并隔离项目数据。

## AE4 - 编辑器承载独立执行流程
- Endpoints: A2, A10
- Label: 双视图共享 nodeId，但关系语义与执行顺序分离。
