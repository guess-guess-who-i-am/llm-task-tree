...
- CurrentResult: 已安装桌面 Prompt 发布器：保存防抖、Codex 临时翻译、逐行结构门禁、备份和 SSH 暂存发布。测试通过；首次真实发布未提交，因 Codex 审批选项位置错误。
- RootCauseAnalysis: Codex exec 的审批参数属于顶层 CLI 选项，不能放在 exec 子命令之后；当前错误发生在模型调用前，现有中英文和远端文件未改变。
- NextIdea: 将审批选项移到 Codex exec 前，重新运行发布器并核验本机与 huangyu 的 Prompt 哈希。
