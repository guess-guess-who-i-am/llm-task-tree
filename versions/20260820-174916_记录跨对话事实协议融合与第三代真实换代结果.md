...
- Approach: 用独占写集与资源租约隔离 Worker，串行集成；接受和 8898 发布分别加锁、校验与回滚。
- Input: 现有并发源码、任务树、官方工程实践和用户方案。
- Output: 隔离设计与门禁见 docs/subtree-parallel/concurrent-agent-isolation-design.zh.md。
- Metrics: 写集和资源零越界；并发接受仅一个成功；发布失败不影响 8898 旧版本。
- Notes: 先完成 P0 资源租约、接受锁和发布状态机；集成 Agent 冲突修复置于 P1。
- CodeLoc:
- CurrentResult: 代码隔离、独占写集、4路调度、串行集成与接受前校验已验证，4组回归通过；资源租约、跨运行接受锁和8898发布状态机尚缺，不能声称完全互不干扰，根目标未达成。
- RootCauseAnalysis: 工作树不能隔离端口、数据库、缓存和发布环境；写集检查也不能替代跨运行的接受与发布锁。
- CaseStudy:
