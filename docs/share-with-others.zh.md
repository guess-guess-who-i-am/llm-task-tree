...
装了 codex CLI 的人还可以 `codex plugin marketplace add guess-guess-who-i-am/llm-task-tree` 从 Git 市场取插件（仓库根的 `.agents/plugins/marketplace.json` 指向 `./marketplace/plugins/task-tree`）。Git 快照登记由该命令自己管理，安装脚本只写已验证的本地市场形态（`source_type = "local"`），不伪造 Git 字段。
