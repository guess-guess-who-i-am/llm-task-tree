...
- Approach:
  - 保留任务树为核心，用同一份真实数据做四个只读前端版本。
  - 四版分别检验：方向主干、阶段泳道、焦点透镜、空间地图。
  - 每版只改变浏览与展开方式，不加入已撤销的控制台或收件箱概念。
  - 先完成桌面与手机真实交互测试，再由用户选择正式方向。
- Input:
...
- Notes:
- CodeLoc:
- CurrentResult: 旧 A/B/C 原型已存在，但用户曾选择 A，原样复测不能回答当前哪个版本更好用。新一轮已确定用四种差异更大的任务树浏览方式进行比较；正式界面保持不变，现阶段尚不能宣称找到更好版本。
- RootCauseAnalysis: 第一处偏离发生在视口策略：系统把“展示完整结构”当默认任务，而用户多数时候执行的是围绕当前节点的局部判断。仅靠加粗主干仍保留了全部竞争刺激，没有按当前兴趣动态选择上下文。
- CaseStudy:
- NextIdea: 完成四版 V2 原型与切换页，跑桌面和手机真实交互测试并交给用户比较。
- SelectedSkills:
