...
- Approach:
  - 任务树保持产品核心和统一数据源，不退化为普通看板或附属视图。
  - 用稳定根目标、活动路径、局部邻域和渐进展开降低阅读与恢复成本。
  - 先用真实树并行比较结构原型，用户测试后再重写正式界面。
- Input:
- Output: 3 个任务树只读前端原型与桌面、窄屏截图；入口 public/task-tree-prototype/index.html，证据见 artifacts/task-tree-prototype-*.png。
- Metrics: 研究覆盖外部认知、信息觅食、中断恢复、自主性与人机协作；设计假设可用定向、恢复、纠错和信任校准对照验证，阈值由基线与错误代价确定。
- Notes:
- CodeLoc:
- CurrentResult:
  - 已把方向修正为“任务树仍是核心，舒适性来自观察和展开方式”，不再让任务树退居普通视图之一。
  - 完成方向脊柱、局部镜头、折叠地形 3 个真实树原型，可用左右键或参数切换。
  - 6 组桌面/窄屏测试均无控制台错误和页面级横向溢出；正式界面与树数据未改。
- RootCauseAnalysis: 上一轮把竞品的分层视图推成“任务树只是其中一个视图”，偏离了本项目以任务树为核心的差异化。根因是把降低阅读成本误成降低树的主导地位。
- CaseStudy:
- NextIdea: 请用户比较 A/B/C 的方向感、恢复速度和下一步判断，再决定保留一个版本或组合其局部设计。
- SelectedSkills:
