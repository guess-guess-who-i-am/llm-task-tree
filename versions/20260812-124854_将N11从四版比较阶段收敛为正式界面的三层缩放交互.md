...
- Approach:
  - 用同一份真实任务树提供四个只读版本，分别检验方向主干、阶段泳道、焦点透镜和空间地图。
  - 各版都让节点回答解决的问题、采用的思路和已有结果，只改变浏览、定位与展开方式。
  - 正式界面保持不变；用户实测选择后，再把有效交互移入正式界面。
- Input:
...
- Notes:
- CodeLoc:
- CurrentResult: 四个只读版本已使用当前14节点真实树运行；桌面与手机共8组交互验收均无控制台错误、横向溢出或可见碰撞。用户现在可直接比较浏览方式，但尚未完成实际选择，因此不能宣称已找到最舒适版本。
- RootCauseAnalysis: 第一处偏离发生在视口策略：系统把“展示完整结构”当默认任务，而用户多数时候执行的是围绕当前节点的局部判断。仅靠加粗主干仍保留了全部竞争刺激，没有按当前兴趣动态选择上下文。
- CaseStudy:
- NextIdea: 请用户围绕同一个感兴趣节点依次试用四版，选出最容易理解局面并决定下一步的版本。
- SelectedSkills:
