...
- Approach:
  - 总览先展示根本目的、整体方向、方向性进展与下一次推进；执行入口和节点索引按需展开。
  - Agent 每轮先读取 ROOT 目的、方向和成功标准，再判断局部动作是否有依据；无依据不补写因果。
  - CurrentResult 只写经证据验证的方向性结论；具体实现留在 NextIdea 与步骤证据。
- Input:
- Output: 方向性项目总览与目的锚定提示；证据见 public/app.js、server/turn-context.js、server/codex-prompts.js、scripts/test-project-overview-ui.mjs。
- Metrics: 首屏只显示目的、方向、方向性进展、一个待解决项和下一次推进；节点索引默认折叠；Agent 提示含 ROOT 目的、方向、成功标准与证据门禁；桌面和 390px 无横向溢出。
- Notes:
- CodeLoc:
- CurrentResult:
  - 项目状态入口已从局部节点清单收敛为 ROOT 目的、整体方向、方向性进展和下一次推进，节点索引默认折叠。
  - 普通 turn 与 Codex 单步提示均注入 ROOT 目的、方向、成功标准和证据门禁，禁止为局部动作编造因果。
  - 原提示功能 21/21 覆盖，短入口 7,167 bytes；MCP 全套、桌面与 390px 总览回归通过。
- RootCauseAnalysis: 阅读阻力与模型幻觉同源于把局部执行字段当作项目状态，同时 checkpoint 未强制局部动作映射回 ROOT。方向状态、具体执行和证据必须分层。
- CaseStudy:
