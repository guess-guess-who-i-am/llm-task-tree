...
- Approach:
  - 保留现有任务树为主界面，把焦点透镜作为深度缩放层，而不是替换整套前端。
  - 进入项目先只显示根本目标、当前进度和当前问题；关闭总览后回到普通树。
  - 放大超过深度阈值时，以选中节点或视口中心节点自动进入透镜；关闭透镜后普通树定位到该焦点节点。
- Input:
...
- Notes:
- CodeLoc:
- CurrentResult: 正式界面已形成三层连续导航：总览只显示根本目标、当前进度和当前问题；普通任务树保留；桌面继续放大时自动进入当前节点透镜，直接显示问题、思路、结果和上下游，关闭后回图并居中。桌面与手机交互验收通过，截图见 artifacts/focus-lens-desktop.png、artifacts/focus-lens-mobile.png。现已具备降低恢复与搜索成本的界面能力，但仍缺用户真实使用反馈，根本目标暂不能宣称完全达到。
