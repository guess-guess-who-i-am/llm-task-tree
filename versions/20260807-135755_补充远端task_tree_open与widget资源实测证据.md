...
- CurrentResult: 本机与远端验证通过：5633 的 /api/project 根目录为 /data1/hyf/platfrom1；MCP、Linux kit、share-install 回归全通过；app-server 重启后新会话已启动 task-tree MCP，task_tree_open 尚在等待模型回合返回。
- RootCauseAnalysis: 原故障是旧 app-server 未加载 8 月 7 日安装的 MCP/plugin，且临时 shell 启动留下 35595 失效标记；现已修复为 app-server 重启、端口标记自愈和 Linux 入口权限恢复。
- NextIdea: 在远端新开或恢复一个 Codex 回合调用 task_tree_open；若宿主仍不显示，用本机 15633 转发验证同一 widget。
