...
- Input:
- Output:
  - docs/agent-context-research/technology-landscape-2026.md # 20 类技术适配研究与 66 个一手链接
  - public/index.html、public/app.js、public/styles.css # 决策总览
  - scripts/test-project-overview-ui.mjs、artifacts/project-overview-*.png # 交互测试与截图证据
- Metrics: 打开后可在一屏识别目标、当前局面、阻塞、推荐下一步；只显示必要候选和一行节点索引；可直接设置 Next 或定位节点；桌面与 390px 无横向溢出。
- Notes:
- CodeLoc:
- CurrentResult:
  - 决策总览已把 13 个节点压成一屏索引，并通过桌面与 390px 交互测试。
  - 20 类技术调研表明，当前优先缺口是工作单元状态机、行为评测、统一追踪和四层护栏；详细证据见 docs/agent-context-research/technology-landscape-2026.md。
  - 保留现有任务树、MCP、RAG 和单写者 coordinator；完整 GraphRAG、独立向量库、微调、蒸馏及大型 Agent 框架暂缓。
- RootCauseAnalysis: 隔天阅读困难源于默认入口按存储字段组织；Agent 长程漂移则源于上下文选择、工作单元提交、评测与追踪分散。两者应由决策总览和可测运行闭环解决，而不是继续堆框架。
- CaseStudy:
- NextIdea: 先设计 agent-run/v1 与 work-unit/v1：定义阶段、事件、停止条件和核心行为评测，再接入普通 turn、chain 与 coordinator。
- SelectedSkills:
