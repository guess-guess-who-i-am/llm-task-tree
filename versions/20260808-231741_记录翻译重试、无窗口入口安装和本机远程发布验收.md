...
- CurrentResult:
  - 22:03 的新保存被正常检测，但两次 Codex 翻译子进程 exit 1，中文源与已发布版本产生差异。
  - 同一载荷未改内容再次调用可生成完整结构化翻译，排除中文结构、Hook 和 SSH 的确定性故障。
  - 发布器已加入三次翻译重试；快捷方式改为无窗口启动器，重试、隐藏入口及原有回归均通过。
- RootCauseAnalysis: 同一载荷可无修改成功，说明本次是 Codex/模型瞬时退出；旧发布器把每次子进程失败直接终止，且桌面快捷方式直接启动可见 PowerShell。
- NextIdea: 重装本机发布器并发布当前中文源，核对快捷方式目标、本机与 huangyu 哈希及发布后无漂移状态。
