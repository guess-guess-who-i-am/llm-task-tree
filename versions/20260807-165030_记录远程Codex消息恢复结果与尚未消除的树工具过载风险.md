...
- CurrentResult:
  - 4152 实际经 Windows SSH 反向隧道转到本机 4000；20 秒最小请求仍为零字节。
  - 本机代理配置 3 个并发槽，但日志显示新请求等待槽位 900 秒后才失败。
  - 4141 是 syf 的另一套代理，与 fqd 当前链路无关。
- RootCauseAnalysis: 故障位于 Windows 4000 代理：并发槽被长请求长期占用，且 900 秒排队超时把故障表现为持续思考；SSH、sudo、task-tree 均已排除。
- NextIdea: 将代理排队上限降到 60 秒并重启清空失效槽位，再依次验收本机 4000、远程 4152 和远程 Codex。
