...
- Notes:
- CodeLoc:
- CurrentResult: 目标是让 Codex 维护任务图时用户可快速选择会话。真实嵌入测试中首个可选项约 150 毫秒出现，14 个可点击项行宽 406px，行间及标题/时间均无重叠；接口连续请求约 7–10 毫秒。会话列表现默认读取持久缓存，昂贵扫描改为手动刷新；当前项目可宣称达到，其他已安装项目尚未发布验证。
- RootCauseAnalysis: 嵌入模式的通用工具栏规则把菜单按钮压成 30px；会话列表又在启动时调用机器级 thread/list，面对约 1183 个会话文件和 10GB 状态会等待 30–45 秒。根因是菜单内容误继承工具栏尺寸，且昂贵刷新被放进自动交互路径。
- CaseStudy:
- NextIdea: 把已验证的会话缓存和嵌入菜单修复发布到所有已安装项目，并抽测本机其他项目与远程 Linux。
- SelectedSkills:
- Folded: true
