...
- Input:
- Output: 人类舒适协作模型、设计推论与 5 篇开放论文；见 docs/agent-context-research/human-capability-comfort.md。
- Metrics: 研究覆盖外部认知、信息觅食、中断恢复、自主性与人机协作；设计假设可用定向、恢复、纠错和信任校准对照验证，阈值由基线与错误代价确定。
- Notes:
- CodeLoc:
- CurrentResult:
  - 人的能力应刻画为借助外部表示进行识别、比较、操纵和线索恢复，而非被动读取固定容量的信息。
  - 舒适意味着减少记忆、搜索与恢复成本，同时保留关键判断和控制权；少字不是独立目标。
  - 已核查 14 项关键研究并保存 5 篇开放论文；ROOT 的 30 秒阈值已改为待基线验证的评价方法。
- RootCauseAnalysis: 此前把“少字、快速读完”当作舒适性的代理，仍在要求人维护模型的内部结构。根因是忽略了外部认知、直接操纵、恢复线索和自主性。
- CaseStudy:
- NextIdea: 先把七类舒适能力转成总览交互假设与现有界面的对照任务，明确可观察指标后再决定是否改 UI。
- SelectedSkills:
